...
from swag_transformers.swag_modernbert import SwagModernBertConfig, SwagModernBertModel, SwagModernBertPreTrainedModel, \
    SwagModernBertForSequenceClassification
from swag_transformers.trainer_utils import SwagUpdateCallback



class TestSwagModernBert(unittest.TestCase):

    def test_untrained(self):
        hidden_size = 240
        config = SwagModernBertConfig(no_cov_mat=False, hidden_size=hidden_size)
    ...
    def test_untrained_classifier(self):
        hidden_size = 240
        num_labels = 3
        config = SwagModernBertConfig(no_cov_mat=False, hidden_size=hidden_size, num_labels=num_labels)
        logging.debug(config)
        swag_model = SwagModernBertForSequenceClassification(config)
        swag_model.sample_parameters()
        logging.debug(swag_model)
        ...
        print(swag_model.swag.base.model.embeddings.tok_embeddings.weight_mean)


class TestSwagModernBertFinetune(unittest.TestCase):
    ...
    def pretrained_bert_classifier_finetune(self, no_cov_mat, module_prefix_list=None):
        ...
        swag_model = SwagModernBertForSequenceClassification.from_base(
    ...
    def finetuned_model_test(self, swag_model, no_cov_mat=True, blockwise=False, scale=1):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            stored_model = SwagModernBertForSequenceClassification.from_pretrained(tempdir).to(self.device)
    ...
    def test_pretrained_bert_classifier_finetune_no_cov_partial(self):
        model = self.pretrained_bert_classifier_finetune(
            no_cov_mat=True, module_prefix_list=['model.embeddings.tok_embeddings', 'classifier'])
        self.finetuned_model_test(model, no_cov_mat=True, blockwise=False, scale=0)  # SWA
        self.finetuned_model_test(model, no_cov_mat=True, blockwise=False, scale=1)  # SWAG-Diag
